...
export function GroceryList({ tripId, user }: GroceryListProps) {
  ...
  const getItemsByCategory = (category: string) => {
    return groceryItems.filter((item: GroceryItemWithDetails) => item.category === category);
  };
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-lg font-bold">${groceryBill.totalCost.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-lg font-medium">Cost Per Person:</span>
                    <span className="text-lg font-bold">${groceryBill.costPerPerson.toFixed(2)}</span>
                  </div>
                  <div className="border-t pt-4">
                    <h4 className="font-medium mb-2">Items by Category:</h4>
                    <div className="space-y-2">
                      {categories.map((category) => {
                        const items = groceryBill.items.filter((item: GroceryItemWithDetails) => item.category === category);
                        if (items.length === 0) return null;
